...
const MyLearning = () => {
    ...
    const filteredEnrollments = enrollments.filter(enrollment => {
        const matchesSearch = enrollment.course_details.title.toLowerCase().includes(search.toLowerCase());
        const matchesFilter =
            filter === 'all' ? true :
                filter === 'completed' ? enrollment.progress === 100 :
                    filter === 'in_progress' ? enrollment.progress < 100 && enrollment.progress > 0 :
                        true; // Default to all if weird state

        // Note: You might want to refine 'in_progress' to include 0 if they just started. 
        // Usually 'in_progress' means they haven't finished. 
        // Let's adjust logic:
        if (filter === 'completed') return matchesSearch && enrollment.progress === 100;
        if (filter === 'in_progress') return matchesSearch && enrollment.progress < 100;
            ...
            {filteredEnrollments.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredEnrollments.map((enrollment) => (
                        <div key={enrollment.id} className="glass rounded-[2rem] overflow-hidden flex flex-col group hover:bg-white/5 transition-all">
                            <div className="relative h-40 overflow-hidden">
                                {enrollment.course_details.thumbnail ? (
                                    <img
                                        src={`http://localhost:8000${enrollment.course_details.thumbnail}`}
                            ...
                            <div className="p-6 flex-1 flex flex-col">
                                ...
                                <div className="flex items-center gap-3 text-xs text-slate-400 mb-6">
                                    ...
                                    <div>
                                        {enrollment.course_details.instructor_name}
                                    </div>
                                </div>
                                ...
                                <div className="mt-auto space-y-4">
                                    <div>
                                        <div className="flex justify-between text-xs font-bold mb-2">
                                            <span className={enrollment.progress === 100 ? "text-emerald-400" : "text-slate-300"}>
                                                {enrollment.progress === 100 ? 'Completed' : `${Math.round(enrollment.progress)}% Complete`}
                                            </span>
                                            <span className="text-slate-500">
                                                {enrollment.progress === 100 ? <CheckCircle size={14} className="text-emerald-500" /> : <TrendingUp size={14} />}
                                            </span>
                                        </div>
                                    ...
                                    <Link
                                        ...
                                        className={`flex items-center justify-center gap-2 w-full py-3 rounded-xl font-bold text-sm transition-all ${enrollment.progress === 100
                ...
                <div className="text-center py-20 glass rounded-[2rem] border border-dashed border-white/10">
                    ...
                    <h2 className="text-2xl font-bold text-white mb-2">No active courses</h2>
